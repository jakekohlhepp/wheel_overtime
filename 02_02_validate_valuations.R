...
library('kableExtra')
library('fixest')
## take date fixed effects and compare them to weather, holidays, and special events.
...
write.csv(special[!is.na(r_date_fe), c("r_date_fe","analysis_workdate", "event_list")], file="analysis/data/02_02_val_special_events_blank.csv")

withnames<-fread('analysis/data/02_02_val_special_events_filled.csv')
...
kable(withnames, "latex", align="c", booktabs=TRUE,linesep = c(""), escape = F, caption = NA, label=NA) %>%
  cat(., file = "analysis/out/tables/02_02_top10_fixed_effects.tex")
...
kable(fortable[,c("Type of Day", "Number of Dates","Avg. Fixed Effect", "Avg. Overtime Shifts") ], "latex", align="c", booktabs=TRUE,linesep = c(""), escape = F, caption = NA, label=NA) %>%
  cat(., file = "analysis/out/tables/02_02_rain_dow_fe.tex")
