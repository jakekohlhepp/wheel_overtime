...
### fact 3: dispersion in contact.
q_degree<-quantile(all_pairs$l_degree,c(0.25, 0.75), na.rm=TRUE)
raw_sd<-sd(all_pairs[!is.na(l_degree)]$l_degree)
print(q_degree["75%"]/q_degree["25%"])

print(sd(resid(feols(l_degree~0|num_emp1,all_pairs)))/raw_sd)


ggplot(all_pairs, aes(x=l_degree))+
  geom_histogram(fill="black")+xlab("Fraction of Days Working Overtime")+ylab("Officer Count")+
  theme_bw()+
  theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank(),axis.title= element_text(size = 30),axis.text = element_text(size = 15))
...
ggplot(all_pairs, aes(x=l_degree-bar_degree))+
  geom_histogram(fill="black")+xlab("Fraction of Days Working Overtime")+ylab("Officer Count")+
  theme_bw()+
  theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank(),axis.title= element_text(size = 30),axis.text = element_text(size = 15))

ggsave("analysis/out/figures/01_00_ot_dipersion_frac_idioscrynatic.png", width=12, height=8, units="in")
...
## residualize.
all_pairs[, opp:=1-dist_from_med]

work_res<-feols(ot_work~opp+normal_work+expected_earnings+seniority_rank|num_emp1+analysis_workdate, data=all_pairs[!is.na(l_wheel_degree),], cluster="num_emp1")
instrument_res<-feols(l_wheel_degree~opp+normal_work+expected_earnings+seniority_rank|num_emp1+analysis_workdate, data=all_pairs[!is.na(l_wheel_degree),], cluster="num_emp1")
