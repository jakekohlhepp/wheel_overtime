## estimate lagged versions
library('fixest')
library('data.table')
set.seed(633491)
all_pairs<-readRDS("analysis/data/00_02_estimation_sample.rds")

### lagged potential suppliers should not matter.
...
etable(res1,res2,res3,res4,res5, fitstat=~r2,keep="!Constant",order="Supplier",dict=c(s_suppliers = "Std. Supplier Count",
       ...
       file="analysis/out/tables/01_02_lag_regs_suppliers.tex", replace=TRUE,signifCode=c(`***`=0.001,`**`=0.01, `*`=0.05))


### lagged connectedness should matter.
setorder(all_pairs, "num_emp1", "analysis_workdate")
...
etable(res1,res2,res3,res4,res5, fitstat=~r2,keep="!Constant",order="Connectedness", dict=c(s_degree = "Std. Connectedness",
                                                                     ...
                                                                     normal_work="Normal Work", an_age="Age" ,seniority_rank="Seniority Rank",ot_work="Overtime",
                                                                     expected_earnings="Expected Earnings",analysis_workdate="Date", num_emp1="Officer"),
       file="analysis/out/tables/01_02_lag_regs.tex", replace=TRUE,signifCode=c(`***`=0.001,`**`=0.01, `*`=0.05))
