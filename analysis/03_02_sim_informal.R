...
## do for different multiples of access costs.
library('data.table')
library('alpaca')
library('lubridate')
set.seed(54323)
...
for (mult in c(1,1/(2:10),2:10)){
  ## utility is sum of all components including wages and degrees.
  all_pairs[, det_util:=mult*(opp_dist*coef(mod_mod)["opp_dist"]+suppliers_interacted*coef(mod_mod)["suppliers_interacted"])+date_fe+officer_fe+
              seniority_rank*coef(mod_mod)["seniority_rank"]+
              normal_work*coef(mod_mod)["normal_work"]+ot_rate*coef(mod_mod)["ot_rate"]]
  
  #### for all multipliers other than 1, need to adjust intercept to maintain similar aggregate number of shifts.
  if (mult!=1){
...
for (iter in 1:max_iter){
  ...
  all_pairs[, true_valuation:=(true_utility - 
              (ot_rate*coef(mod_mod)["ot_rate"] +mult*(opp_dist*coef(mod_mod)["opp_dist"]+suppliers_interacted*coef(mod_mod)["suppliers_interacted"]) ))/(coef(mod_mod)["ot_rate"]) ]
  ...
  saveRDS(results, "analysis/data/03_02_sim_informal.rds")
  saveRDS(results_byworker, "analysis/data/03_02_sim_informal_byworker.rds")
