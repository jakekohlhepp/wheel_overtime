...
load("analysis/data/02_00_estimate_probit.Rdata")
...
kable(output[,-"type"], "latex", align="c", booktabs=TRUE,linesep = c(""), escape = F, caption = NA, label=NA) %>%
  cat(., file = "analysis/out/tables/02_01_main_estimates.tex")
...
ggsave("analysis/out/figures/02_01_officer_fe.png", width=10, height=10, units="in")

ggplot(date_fe, aes(x=date_fe))+
  geom_histogram(fill="black")+xlab("Earnings Equivalent Date Fixed Effect ($)")+ylab("Officer Count")+
  theme_bw()+
  theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank(),axis.title= element_text(size = 30),axis.text = element_text(size = 30))

ggsave("analysis/out/figures/02_01_date_fe.png", width=10, height=10, units="in")
