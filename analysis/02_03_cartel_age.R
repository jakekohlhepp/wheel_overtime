...
### count ot, get safety ability, get valuation, get average network position
...
by_emp<-all_pairs[, .(observed_ot_count=sum(ot_work),valuation=unique(valuation),
                      ...
                      ), by=c("num_emp1","has_family")]

## convert fe to wage equivalents and display format.

by_emp[, rank_safety:=frank(safety_ability,ties.method="dense")]
...
kable(cartel, "latex", align="c", booktabs=TRUE,linesep = c(""), escape = F, caption = NA, label=NA) %>%
  ...
  cat(., file = "analysis/out/tables/02_03_cartel.tex")
...
ggsave("analysis/out/figures/02_03_age_valuation.png", width=12, height=8, units="in")
