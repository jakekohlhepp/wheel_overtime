## simulate a savvy manager who can make only a certain number of swaps.
...
## adjust det_value by maximum connectedness
all_pairs[, det_val:=det_val+max(l_degree, na.rm=TRUE)*coef(mod_mod)["l_degree"]]
...
do_sim<-function(x){
  ...
  hold<-copy(all_pairs)
  hold[, rand:=runif(.N)]
  hold[, true_valuation:=(det_val+rlogis(.N))/(coef(mod_mod)["max_rate"]/1.5)]
  ...
  for (looper in 1:600){
    for (d in unique(hold$analysis_workdate)){
      ...
      hold[analysis_workdate==d,true_valuation:= bubble_sort(true_valuation,swap_count),]
      hold[analysis_workdate==d,num_emp1:= bubble_sort_name(true_valuation,num_emp1,swap_count),]
...
for (i in 1:50){
  ...
  saveRDS(all_res, "analysis/data/03_03_sim_manager_values_cluster.rds")
