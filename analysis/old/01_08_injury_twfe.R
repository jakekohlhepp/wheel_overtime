...
all_pairs[is.na(injured), injured:=0]
## find family leave
all_pairs[injured==1, first_time:=min(analysis_workdate), by="num_emp1"]
all_pairs[injured==1, is_leave:= first_time==analysis_workdate ]
all_pairs[injured==0, is_leave:= 0 ]
...
for (tt in 1:nrow(term_list)){
  ...
  mark_list<-names(mark_list)[which(mark_list>=5)]
...
png("analysis/out/figures/01_08_injury_twfe.png", width=900, height=500)
iplot(twfe,main="",xlab="Time to Treatment (Days)",ylab="Connectedness", drop = "([9]\\d{1}|\\d{3})", lab.fit="simple")
dev.off()
