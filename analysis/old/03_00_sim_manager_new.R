...
do_sim<-function(x){
  ...
  for (savy in 1:200){
    ...
    hold[,new_list:= bubble_sort(safety_ability,savy), by="analysis_workdate"]
    
    hold[,sim_prod:= (1:.N<=tot_ot_among)*new_list, by="analysis_workdate"]
    results<-rbind(results, data.table(savy_num=savy,sim_num=x,prod=sum(hold$sim_prod)))
  }
  return(results)
}
temp_res<-data.table(do.call(rbind,mclapply(1:2000,do_sim, mc.cores=core_count)))

saveRDS(temp_res, "analysis/data/03_00_sim_manager_cluster.rds")
