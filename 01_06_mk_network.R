...
to_replace<-paste0("helper_",colnames(all_pairs)[grep("^[0-9]+$",colnames(all_pairs))])
...
check<-all_pairs[,.SD, .SDcols=to_replace]
...
##3 remove helper vars
all_pairs[,(to_replace):=NULL ]

#### redo but no reweight by those who do not have wheel assignment
...
## check to see how multiplication is occuring. change the name of variables to see.
to_replace<-paste0("helper_nothave_",colnames(all_pairs)[grep("^[0-9]+$",colnames(all_pairs))])
all_pairs[, (to_replace):=fold_mult(.SD), .SDcols=c(colnames(all_pairs)[grep("^[0-9]+$",colnames(all_pairs))],colnames(temp)[-1])]
...
## all the na lagged wheel degrees are the first days of officers.
stopifnot(all_pairs[is.na(l_wheel_nothave_degree), ]$first)
